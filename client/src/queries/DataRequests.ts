...
export const GetSegmentsAndAverageValuesInAPolygon = async (
  points: LatLng[],
  type: string,
  aggregation: string,
  direction: number,
...
export const GetAggregationTypes = async (
  dataType: string,
): Promise<string[]> => {
...
export const GetDataTypesOfSegment = async (
  segment_id: number,
): Promise<string[]> => {
...
export const GetAggregationTypesOfSegment = async (
  dataType: string,
  segment_id: number,
): Promise<string[]> => {
  const path =
    '/types/aggregatedValues/aggregation/' + segment_id + '?type=' + dataType;
  const res = await fetch(path);
  const data = await res.json();
  return data.map((elt: any) => elt.Aggregation);
};

export const GetSegmentAndAggregateValue = async (
  type: string,
  aggregation: string,
  segment_id: number,
): Promise<Segment> => {
  const path =
    '/segments/' + segment_id + '?type=' + type + '&aggregation=' + aggregation;
  console.log(path);
  const res = await fetch(path);
  const data = await res.json();
  return data;
