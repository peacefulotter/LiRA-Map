...
const TypesPopup: FC<IPopupWrapper> = ({ types, setTypes }) => {
  ...
  const direction = -1;
  const fetchAggrTypes = (dt: string) =>
    GetAggregationTypes(dt).then((at) => {
      console.log(dt, at);
      setAggrTypes(at);
    });
  ...
  useEffect(() => {
    if (dataType !== undefined) fetchAggrTypes(dataType);
  }, [dataType]);
    ...
    <div className="seg-popup-container">
      ...
      {dataType !== undefined ? (
        <Checkboxes
          ...
          types={aggrTypes}
          type={aggrType}
          onClick={aggregationTypeOnClick}
        />
      ) : null}
    </div>
  );
