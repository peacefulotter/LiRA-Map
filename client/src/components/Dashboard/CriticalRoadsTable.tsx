...
const color_red = 'rgba(255, 23, 68, 0.7)';
const color_yellow = 'rgba(255, 199, 0, 0.8)'
const color_green = 'rgba(29, 204, 0, 0.8)';
...
export default function CriticalRoadsTable() {
    ...
    const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.checked) {
            const newSelected = rows.map((n) => n.roadId);
            ...
            <Paper sx={{ width: '100%', mb: 2 }}>
              ...
              rows.sort(getComparator(order, orderBy)).slice() */}
                            {stableSort(rows, getComparator(order, orderBy))
                                ...
                                .map((row, index) => {
                                    const isItemSelected = isSelected(row.roadId);
                                    const labelId = `enhanced-table-checkbox-${index}`;

                                    return (
                                        <TableRow
                                            hover
                                            onClick={(event) => handleClick(event, row.roadId)}
                                            role="checkbox"
